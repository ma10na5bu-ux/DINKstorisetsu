...
const WP_URL = process.env.WP_URL || 'https://dinks-torisetsu.fun';
...
const SSP_RECOMMENDED = {
  ...
  // ホームページSEO
  home: {
    title:       'DINKsのトリセツ｜共働き夫婦のためのライフスタイルメディア',
    description: '共働き子なし夫婦のためのメディア「DINKsのトリセツ」。お金・暮らし・旅行・キャリア情報をDINKs目線でお届けします。',
    noindex:     '0',
  },
};
...
function showManualGuide() {
  ...
  const sections = [
    ...
    },
    {
      ...
      icon: '📝',
      items: [
        { label: '投稿(post) title形式', value: '%title% | DINKsのトリセツ', note: '' },
        { label: '投稿 description形式', value: '%description%（自動抜粋）', note: '' },
        { label: '固定ページ title形式', value: '%title% | DINKsのトリセツ', note: '' },
        { label: 'カテゴリー title形式', value: '%term_title%の記事一覧 | DINKsのトリセツ', note: '' },
      ],
    },
    {
      step: 'STEP 6',
...
async function main() {
  ...
  console.log('');
  console.log('DINKsmedia SEO SIMPLE PACK 設定ツール');
