#!/usr/bin/env node
/**
 * Gemini Imagen 4 でDINKs記事用アイキャッチ画像を生成し
 * WordPressにアップロードするスクリプト
 *
 * 使い方:
 *   node gemini-image-generate.js "DINKs週末ブランチ"
 *   node gemini-image-generate.js "DINKs週末ブランチ" --post-id=8
 */
...
function buildPrompt(title) {
  return `A high-quality, warm and stylish photograph of a happy couple (DINKs - dual income no kids) in a modern Japanese lifestyle setting.
Context: ${title}
Style: Natural photography, soft lighting, vibrant colors, 16:9 aspect ratio.
The couple should look relaxed and enjoying their lifestyle. Modern Japanese interior or urban scenery in the background.
...
async function generateImage(title) {
  ...
  // 一時ファイルに保存
  const tmpPath = path.join('/tmp', `dinks-${Date.now()}.png`);
...
async function uploadToWordPress(imagePath, title) {
  ...
  const filename = `dinks-${title.replace(/[^a-zA-Z0-9ぁ-ん一-龯]/g, '-')}-${Date.now()}.png`;
