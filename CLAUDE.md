...
- **①②③番号段落は使わない**。「ポイント（短く・スキャン可能）＋理由（括弧内補足）」構造の箇条書きに変換する
  - NG: `①強度やコストを自分で調整できる趣味を選ぶ。時間が読めない30代にとって〜`
  - OK: `- 強度やコストを自分で調整できる趣味を選ぶ（通えない週があっても続けられるかどうかが重要）`
  - 理由：番号段落は「ポイント」と「説明」が同じ段落に混在し、視線が止まる場所がなくなる
